...
Для реализации такого подхода достаточно выдать галерее класс `incative`, который сменится на `active` во время инициализации, и написать два набора стилей для обоих состояний.
