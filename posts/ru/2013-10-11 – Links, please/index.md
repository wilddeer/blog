...
<%- include('/svg/history.svg') %>Примеры, на которые ссылается пост, давно протухли, но смысл поста до сих пор актуален.
...
<%- include('/svg/umbrella.svg') %>Можно также использовать любой другой тег, например `span`, но он не будет ловить фокус, а событие *click* не будет вызываться по нажатию энтера, что убивает все преимущества кнопки в плане доступности использования.
