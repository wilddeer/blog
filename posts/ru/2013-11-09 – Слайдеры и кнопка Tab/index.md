...
<%- include('/svg/code.svg') %>Выше используется простая универсальная функция `addEvent`{.is-colored-bg}:
...
<%- include('/svg/history.svg') %>Хреновый способ убрать аутлайн, не стоит сбрасывать позицию фокуса. Лучше [воспользуйтесь этим методом](/Про_аутлайны/){.is-colored-bg}.
